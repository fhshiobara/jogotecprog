...
void Jogo::executar() { 
    ...
    // Demonio* inimigoM = new Demonio(CoordF(600.0f, 200.0f), 100, 80.0f, 4);
    // inimigoM->initialize();

    Morte* boss = new Morte(CoordF(600.0f, 200.0f), 100, 80.0f, 4);
    boss->initialize();
    ...
    while (pGerenciadorGrafico->windowopen()) {
        ...
        // inimigoM->perseguir(jogador->getPos(), dt);
        // inimigoM->atualizarAnimacao(Animation_ID::walk, inimigoM->estaOlhandoEsquerda(), dt);
        // inimigoM->desenhar();
